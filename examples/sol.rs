...
use arena_space::components::{Population, Facility, MassRate};
...
fn setup() -> SystemState {
    ...
    let humanity = ss.state.nation.create(Nation { name: "The Imperium".to_string() });
    ...
    ss.state.colony.production
        .get_mut(Facility::Farmland)
        .insert_unvalidated(earth_colony, ProductionUnit::new(earth_population.get_food_requirement() * 0.25));
